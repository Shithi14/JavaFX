...
public class SpaceshipRace extends Application {
	...
	public void start(Stage primaryStage) {
		...
		Rectangle menu = new Rectangle(0, 553, 800, 155);
		menu.setArcWidth(10);
		...
		comboBox.setPromptText("Select Spaceship"); // Give the Directions to
													// the user
		comboBox.setLayoutX(620);
		comboBox.setLayoutY(560);
		// Create a Cell Factory to add Colors to the options
		// It also change size and font
		comboBox.setCellFactory(new Callback<ListView<String>, ListCell<String>>() {
			public ListCell<String> call(ListView<String> p) {
				final ListCell<String> cell = new ListCell<String>() {
					{
						// Change option's Font and size
		...
		// You need to create a Changelistener of Objects to whenever the value
		// is change.
		...
		instructionLabel.setLayoutX(350);
		instructionLabel.setLayoutY(560);
		...
		changingLabel.setLayoutX(350);
		changingLabel.setLayoutY(630);
		...
		timerLabel.setLayoutY(530);
		timerLabel.setFont(Font.font("Impact", FontWeight.BOLD, 150));

		//AudioClip sound = new AudioClip("file:Mac Os X:/cool/neon.mp3");
		...
		startButton.setFont(Font.font("Impact", FontWeight.BOLD, 30));
		startButton.setLayoutX(23);
					...
					public void handle(MouseEvent event) {
						...
						try {
							...
							if (bet > 1000) // If value is not valid,
								...
								case "0":
									changingLabel
											.setText("Thanks for Betting!! \n You bet $"
													+ bet
													+ " on the RED Spaceship.");
									break;
								case "1":
									changingLabel
											.setText("Thanks for Betting!! \n You bet $"
													+ bet
													+ " on the PINK Spaceship.");
									break;
								case "2":
									changingLabel
											.setText("Thanks for Betting!! \n You bet $"
													+ bet
													+ " on the GREEN Spaceship.");
									break;
								case "3":
									changingLabel
											.setText("Thanks for Betting!! \n You bet $"
													+ bet
													+ " on the PURPLE Spaceship.");
									break;
								case "4":
									changingLabel
											.setText("Thanks for Betting!! \n You bet $"
													+ bet
													+ " on the BLUE Spaceship.");
									break;
								}
								// Remove old pane
								...
								// the rockets
								Runnable race = new MakeRockets(paneRace,
										finishedOrder);
								...
								Runnable results = new PostResults(pane,
										finishedOrder, changingLabel,
										userChoice, bet, resetButton);
		...
		resetButton.addEventHandler(MouseEvent.MOUSE_CLICKED,
				new EventHandler<MouseEvent>() {
					...
					public void handle(MouseEvent event) {
						...
						//stop the threads from finishing, in case you interrupted a race
						...
						drawRocket(paneRace, -5, 50, 0.6, 360, Color.DEEPPINK,
								finishedOrder, "2", false);
						drawRocket(paneRace, -5, 150, 0.6, 360,
								Color.GREENYELLOW, finishedOrder, "3", false);
						drawRocket(paneRace, -5, 250, 0.6, 360,
								Color.MEDIUMPURPLE, finishedOrder, "4", false);
						drawRocket(paneRace, -5, 350, 0.6, 360,
	...
	public static void loadBackground(Pane pane) {
		...
		Image galaxy = new Image("https://i.ytimg.com/vi/kxfeTcty3Do/maxresdefault.jpg");
		
		// Painting the image
		ImageView galaxyBackground = new ImageView();
		galaxyBackground.setImage(galaxy);
                galaxyBackground.setFitWidth(100);
                galaxyBackground.setFitHeight(100);
	...
	public static void drawMoon(Pane pane, double centerX, double centerY,
		...
		//		"http://lroc.sese.asu.edu/news/uploads/"
		//				+ "M116309931R_thumb.png", 275.0 * scale, 275.0 * scale, false, true,
		//		false);
	...
	public static void drawSun(Pane pane, double centerX, double centerY,
		...
		// Load the image from a specific file
		Image sunSurfaceImage = new Image(
				"http://s2.www.astronomycameras.com/data/blog/"
						+ "20081219/assets/sun240808-surface-dmk41.jpg", scale,
	...
	public static void drawRocket(Group pane, double centerX, double centerY,
		...
		if (runing) {
			...
			//Create a EventHandler to know who finished the race.
			pathTransition.setOnFinished(new EventHandler<ActionEvent>() {
				@Override
				public void handle(ActionEvent event) {
					// When Rocket finish race get positions
	...
	public static void drawTrack(Pane pane) {
		...
		for (int i = 0; i < 49; i++) {
			for (int j = 0; j < 6; j++) {
				...
				star.getPoints()
						.addAll(new Double[] { 15.00 + i * 16, 12.0 + j * 100,
								13.05 + i * 16, 16.5 + j * 100, 9.00 + i * 16,
								16.5 + j * 100, 12.0 + i * 16, 19.5 + j * 100,
								10.5 + i * 16, 24.0 + j * 100, 15.0 + i * 16,
								21.75 + j * 100, 19.5 + i * 16, 24.0 + j * 100,
								18.0 + i * 16, 19.5 + j * 100, 21.0 + i * 16,
								16.5 + j * 100, 17.1 + i * 16, 16.5 + j * 100, });
				...
				// It looks like the stars are moving :D
				FadeTransition ft = new FadeTransition(
						Duration.millis(1000), star); // Fade Duration
				ft.setFromValue(1.0); 
				ft.setToValue(0.1);
				ft.setCycleCount(Timeline.INDEFINITE); // Infinite Fade
				ft.setAutoReverse(false); // Start from Dark Color to light Color Everytime
				ft.play(); // Play FadeTransition
			}
		}
	}

	// The task for printing the results of the race
	class PostResults implements Runnable {
		...
		public PostResults(Pane pane, Queue<String> finishedOrder, Label changingLabel,
				String userChoice, double bet, Button resetButton) {
		...
		public void run() {
			...
			if (finishedOrder.poll().equals(userChoice))
				temp = ("Wow, You Just Won First!! You Earned $" + bet * 1000 + "!!!");
			else if (finishedOrder.poll().equals(userChoice))
		...
		public void run() {
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					drawRocket(pane, -5, 50, 0.6, 360, Color.DEEPPINK,
							finishedOrder, "1", true);
					drawRocket(pane, -5, 150, 0.6, 360, Color.GREENYELLOW,
							finishedOrder, "2", true);
					drawRocket(pane, -5, 250, 0.6, 360, Color.MEDIUMPURPLE,
							finishedOrder, "3", true);
					drawRocket(pane, -5, 350, 0.6, 360, Color.DEEPSKYBLUE,
