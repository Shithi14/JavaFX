...
public class TowerOfHanoi7 extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        for (int i = numDisks; i >= 1; i--) {
            ...
            disk.setTranslateY(ROD_HEIGHT - (numDisks - i) * 30); // Proper vertical spacing
        ...
        Scene scene = new Scene(root, 1050, 700); // Increased width for more space
    ...
    private Line createRod(double x) {
        Line rod = new Line(x, 100, x, 450); // Increased the second y-coordinate (450) for a taller rod
    ...
    private Text createRodLabel(double x, String label) {
        Text text = new Text(x, ROD_HEIGHT + 90, label); // Added the label as the third argument
    ...
    private void moveDisk(int from, int to) {
        ...
        double endY = ROD_HEIGHT - rods[to].size() * 30;
