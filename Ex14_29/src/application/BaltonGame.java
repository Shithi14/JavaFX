...
public class BaltonGame extends Application { // Main class extending JavaFX Application
        ...
        startGameButton.setOnAction(event -> {
            try {
                ...
                for (int i = 0; i < numBalls; i++) {
                    ...
                    Thread.sleep(100); // Pause between ball drops
                }
    ...
    private void dropBall(Pane pane, double startX, double startY, int rows, double slotWidth, double bottomY, int[] slots, int[] finishedBalls, int totalBalls, VBox outputBox) {
        ...
        for (int i = 0; i < rows; i++) {
            ...
            if (random.nextBoolean()) {
                currentX -= slotWidth / 2; // Move left
            } else {
                currentX += slotWidth / 2; // Move right
                slotIndex++; // Increment slot index
            }
            path.getPoints().addAll(currentX, currentY); // Add movement to path
        }

        // Add ball to slot
        slots[slotIndex]++; // Increment slot count for the current slot

        // Calculate the final position of the ball in the slot
        double finalX = currentX;
        double finalY = bottomY - (slots[slotIndex] ) * (BALL_RADIUS * 2); // Adjust finalY based on the slot count
        ...
        pathTransition.setDuration(Duration.seconds(1)); // Animation duration
