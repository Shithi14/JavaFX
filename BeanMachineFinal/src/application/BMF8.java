...
public class BMF8 extends Application {
    ...
    private void dropBallsSequentially(Pane pane) {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                ball.setId("ball" + ballIndex); // Update the ID to be unique
                ...
                // Increase slots[finalSlotIndex] count first to use for Y position calculation
                slots[finalSlotIndex]++;

                // Calculate the final Y position, ensuring balls do not overlap
                double finalY = 790 - slots[finalSlotIndex] * (BALL_RADIUS * 2 + 10); // Adjust spacing more clearly

                // Control point for a smooth curve
                double controlY = finalY - 130; // Adjust for curve shape
                path.getElements().add(new QuadCurveTo(
                ...
                pathTransition.setOnFinished(ev -> {
                    ...
                    // Ensure all labels are updated after the last ball
