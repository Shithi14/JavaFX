...
public class BMF11 extends Application {
    ...
    private void dropBallsSequentially(Pane pane) {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                // Calculate the final slot position for the ball
                ...
                double finalY = 780 - slots[finalSlotIndex] * (2 * BALL_RADIUS + 10);

                // Define path through pegs
                Circle lastPeg = null;
                for (Circle peg : pegs) {
                ...
                // Ensure a smooth transition to the final slot
                if (lastPeg != null) {
                    path.getElements().add(new QuadCurveTo(
                            lastPeg.getCenterX(), lastPeg.getCenterY() + 20,
                            finalX, finalY
                    ));
                }
                ...
                pathTransition.setOnFinished(ev -> {
                    // Correctly position the ball at the end of the animation
                    ball.setCenterX(finalX);
                    ball.setCenterY(finalY);

                    // Increment slot count and update the label
