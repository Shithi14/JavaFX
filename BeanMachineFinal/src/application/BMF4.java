...
public class BMF4 extends Application {
    ...
    private void dropBallsSequentially(Pane pane) {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            // Use a PauseTransition to delay each ball drop
            PauseTransition delay = new PauseTransition(Duration.seconds(i * 1.5));
            delay.setOnFinished(event -> {
                Circle ball = new Circle(450, 100, BALL_RADIUS); // Starting position
                ball.setFill(randomColor());
                ball.setId("ball");
                pane.getChildren().add(ball);

                Path path = new Path();
                path.getElements().add(new MoveTo(ball.getCenterX(), ball.getCenterY())); // Start path at initial position

                double currentX = ball.getCenterX(); // Track ball's X position during movement
                double currentY = ball.getCenterY(); // Track ball's Y position during movement

                // Simulate collision with pegs
                for (Circle peg : pegs) {
                    double bounceDirection = random.nextBoolean() ? SLOT_WIDTH / 4 : -SLOT_WIDTH / 4; // Random left/right bounce
                    currentX = peg.getCenterX() + bounceDirection;
                    currentY = peg.getCenterY() + 20; // Move downward
                ...
                int machineWidth = SLOT_WIDTH * SLOT_COUNT; // Total width of all slots
                int slotStartX = paneCenterX - machineWidth / 2; // Starting X position of slots

                // Determine the slot index based on the ball's final X position
                int slotIndex = Math.max(0, Math.min(SLOT_COUNT - 1,
                        (int) ((currentX - slotStartX) / SLOT_WIDTH))); // Ensure slotIndex stays in bounds

                // Calculate the center of the target slot
                double slotCenterX = slotStartX + slotIndex * SLOT_WIDTH + SLOT_WIDTH / 2;

                // Set final Y position to stack balls above the rectangle
                double finalY = 780 - (slots[slotIndex]) * (BALL_RADIUS * 2 + 5); // Adjust stacking height

                // Smooth transition to the final slot position
                double controlY = finalY - 80; // Control point for the curve
                path.getElements().add(new QuadCurveTo(
                        (slotCenterX + currentX) / 2, controlY, // Smooth curve to final position
                        slotCenterX, finalY
                ));

                // Update slot count to reflect ball placement
                slots[slotIndex]++;
                ...
                pathTransition.setOnFinished(ev -> {
                    if (ballIndex == ballCount - 1) {
                        updateSlotLabels(); // Update slot labels after all balls are dropped
                    }
                });
                pathTransition.play();
            });
