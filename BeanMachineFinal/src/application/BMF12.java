...
public class BMF12 extends Application {
    ...
    private void dropBallsSequentially(Pane pane) {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                path.getElements().add(new LineTo(finalX, 100)); // LineTo used for a direct path

                PathTransition pathTransition = new PathTransition(Duration.seconds(8), path, ball);
                pathTransition.setCycleCount(1);
                pathTransition.setOnFinished(ev -> {
                    // Calculate the final Y position correctly
                    double finalY = 780 - slots[finalSlotIndex] * (BALL_RADIUS * 2 + 10); // This will stack balls without overlap
                    ball.setCenterY(finalY); // Set the final Y position of the ball

                    slots[finalSlotIndex]++; // Increment slots after the animation completes
