...
public class RacingCar12 extends Application {

    private double userCarX = 50, userCarY = 300;
    private double compCarX = 50, compCarY = 400;
    private double userSpeed = 1.0, compSpeed = 2.0;
    private boolean isRaceStarted = false;
    private boolean isRaceFinished = false;

    private final double MAX_USER_SPEED = 4.0;
    ...
    public void start(Stage primaryStage) {
        ...
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long now) {
                if (isRaceStarted && !isRaceFinished) {
                    ...
                    if (Math.random() < 0.05) compSpeed += 0.1; // Gradual increase
                    if (Math.random() < 0.02) compSpeed = Math.max(1.5, compSpeed - 0.2);

                    // Check for speed zones
                    handleSpeedZones();

                    // Check for obstacles
                    if (Math.random() < 0.01) userSpeed = Math.max(MIN_USER_SPEED, userSpeed - 0.3);
        ...
        scene.setOnKeyPressed(e -> {
            ...
            switch (e.getCode()) {
                case UP -> userSpeed = Math.min(MAX_USER_SPEED, userSpeed + 0.5);
                case DOWN -> userSpeed = Math.max(MIN_USER_SPEED, userSpeed - 0.5);
            }
        });

        primaryStage.setTitle("Complex Racing Game");
    ...
    private void handleSpeedZones() {
        ...
        if (compCarX >= 500 && compCarX <= 550) compSpeed = Math.max(1.5, compSpeed - 0.2); // Slow zone
        if (compCarX >= 350 && compCarX <= 400) compSpeed = Math.max(1.5, compSpeed - 0.5); // Reduced braking
    }
