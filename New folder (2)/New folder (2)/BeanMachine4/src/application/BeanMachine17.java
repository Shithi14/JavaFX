...
public class BeanMachine17 extends Application {
	...
	public void start(Stage primaryStage) {
		...
		/*
		 * Apply gradient background Soft Glow:
		 */
		...
		ballInputField
				.setStyle("-fx-background-color: lightblue;" + "-fx-text-fill: black;" + "-fx-prompt-text-fill: blue;"
		...
		startButton.setOnAction(e -> {
			// Remove all existing ball nodes from the pane
			pane.getChildren().removeIf(node -> node instanceof Circle && !pegs.contains(node)
			// Remove circles that are not pegs
			);

			// Reset slot counts and update labels
			for (int i = 0; i < SLOT_COUNT; i++) {
				slots[i] = 0;
			...
			// Start dropping the balls sequentially
	...
	/**********************************
	 * Draw machine section
	 * 
	 * Draw pegs serially as a pattern
	 **********************************/
	 ...
	 * 
	 * Don't drop all balls at a time
	 * 
	 *********************************/
	private void dropBallsSequentially(Pane pane) {
		...
		for (int i = 0; i < ballCount; i++) {
			...
			delay.setOnFinished(event -> {
				...
				// Add bounces for pegs
				for (Circle peg : pegs) {
					double bounceDirection = random.nextBoolean() ? -15 : 15;
					path.getElements().add(new QuadCurveTo(peg.getCenterX(), peg.getCenterY(),
							peg.getCenterX() + bounceDirection, peg.getCenterY() + 20));
				}

				// Determine the final slot
				int finalSlotIndex = random.nextInt(SLOT_COUNT);
				double finalX = 410 - (SLOT_COUNT / 2) * SLOT_WIDTH + finalSlotIndex * SLOT_WIDTH + SLOT_WIDTH / 2;

				// Add the final path to the slot area
				...
				pathTransition.setOnFinished(ev -> {
					// Calculate the final Y position to stack balls without overlap
					double finalY = 780 - slots[finalSlotIndex] * (BALL_RADIUS * 2 + 5);

					// Ensure the ball visually moves to the correct slot
	...
	private Color randomColor() {
		...
		// Calculate brightness to identify light colors
		double brightness = (red + green + blue) / 3;

		// If the color is too light, reduce brightness to make it darker
		if (brightness > 0.6) { // Threshold for light colors
			red *= 0.5;
			green *= 0.5;
			blue *= 0.5;
		}
