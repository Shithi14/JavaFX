...
public class BMF24 extends Application {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                for (Circle peg : pegs) {
                    ...
                    // Fix boundary reflection logic
                    if (newX < minSlotX) {
                        newX = minSlotX; // Reset position to left boundary
                        bounceDirection = Math.abs(bounceDirection); // Ensure bounce moves right
                    } else if (newX > maxSlotX) {
                        newX = maxSlotX; // Reset position to right boundary
                        bounceDirection = -Math.abs(bounceDirection); // Ensure bounce moves left
                    }
                ...
                // Use findNextSlot to ensure balanced filling
    ...
    private int findNextSlot(int currentSlot, int minBalls) {
        ...
        while (true) {
            ...
            // Check if either the left or right slot has the minimum number of balls
            ...
            // Break condition: If no slots with `minBalls` are found (unlikely)
            if (left == 1 && right == SLOT_COUNT - 1) {
                break;
            }
        }

        // Fallback: If no valid slot is found, return the current slot
        return currentSlot;
    }
