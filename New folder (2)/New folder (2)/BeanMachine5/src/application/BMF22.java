...
public class BMF22 extends Application {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                for (Circle peg : pegs) {
                    ...
                    // If ball hits the left boundary
                    if (newX < minSlotX) {
                        newX = minSlotX + Math.abs(minSlotX - newX); // Reflect into bounds
                        bounceDirection = Math.abs(bounceDirection); // Reverse to move right
                    }
                    // If ball hits the right boundary
                    else if (newX > maxSlotX) {
                        newX = maxSlotX - Math.abs(newX - maxSlotX); // Reflect into bounds
                        bounceDirection = -Math.abs(bounceDirection); // Reverse to move left
                    }
                ...
                double finalX = 410 - ((SLOT_COUNT - 1) / 2) * SLOT_WIDTH + 
                                (finalSlotIndex - 1) * SLOT_WIDTH + SLOT_WIDTH / 2;
