...
public class BMF25 extends Application {
	...
	private ArrayList<Circle> drawMachine(Pane pane) {
		...
		for (int i = 0; i < SLOT_COUNT; i++) {
			...
			LinearGradient gradient = new LinearGradient(0, 0, 1, 1, true, CycleMethod.NO_CYCLE, new Stop(0, color1),
					new Stop(1, color2));
		...
		LinearGradient baseGradient = new LinearGradient(0, 0, 1, 0, true, CycleMethod.NO_CYCLE,
	...
	private void dropBallsSequentially(Pane pane) {
		...
		for (int i = 0; i < ballCount; i++) {
			...
			delay.setOnFinished(event -> {
				...
				final int finalSlotIndex = Math.max(1,
						Math.min((int) ((currentX - (450 - (SLOT_WIDTH * (SLOT_COUNT - 1)) / 2)) / SLOT_WIDTH) + 1,
								SLOT_COUNT - 1));

				double finalX = 410 - ((SLOT_COUNT - 1) / 2) * SLOT_WIDTH + (finalSlotIndex - 1) * SLOT_WIDTH
						+ SLOT_WIDTH / 2;
