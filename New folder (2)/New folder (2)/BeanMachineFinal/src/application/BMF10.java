...
public class BMF10 extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        startButton.setOnAction(e -> {
            ...
            pane.getChildren().removeIf(node -> 
                node instanceof Circle && !pegs.contains(node) // Remove circles that are not pegs
            );
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                for (Circle peg : pegs) {
                    ...
                    double nextX = peg.getCenterX() + bounceDirection;
                    double nextY = peg.getCenterY() + 20;
                    path.getElements().add(new QuadCurveTo(peg.getCenterX(), peg.getCenterY(), nextX, nextY));
                }

                // Determine the final slot index
                int finalSlotIndex = random.nextInt(SLOT_COUNT);

                // Calculate the final X position of the ball
                double slotWidth = SLOT_WIDTH;
                double slotStartX = 410 - (SLOT_COUNT / 2) * slotWidth; // Centered slots
                double finalX = slotStartX + finalSlotIndex * slotWidth + slotWidth / 2;

                // Calculate the final Y position of the ball
                slots[finalSlotIndex]++;
                double finalY = 780 - (slots[finalSlotIndex] - 1) * (BALL_RADIUS * 2 + 10);

                // Add a smooth transition from the last peg to the final slot
                path.getElements().add(new QuadCurveTo(
                        (finalX + ball.getCenterX()) / 2, 600, // Control point for smooth curve
                        finalX, finalY
                ));
                ...
                pathTransition.setOnFinished(ev -> {
                    // Update the slot label to reflect the new ball count
