...
public class BMF5 extends Application {
    ...
    private void dropBallsSequentially(Pane pane) {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                // Calculate final slot and path
                double finalX = 450 - (SLOT_COUNT / 2) * SLOT_WIDTH + random.nextInt(SLOT_COUNT) * SLOT_WIDTH + SLOT_WIDTH / 2;
                double finalY = 780 - slots[(int) ((finalX - 450 + (SLOT_COUNT / 2) * SLOT_WIDTH) / SLOT_WIDTH)] * 20;
                ...
                pathTransition.setOnFinished(ev -> {
                    ...
                    if (ballIndex == ballCount - 1) {
                        // Refresh labels after last ball
                        updateSlotLabels();
                    }
                });
                pathTransition.play();
