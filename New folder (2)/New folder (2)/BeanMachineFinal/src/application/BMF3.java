...
public class BMF3 extends Application {
        ...
        for (int i = 0; i < ballCount; i++) {
            ...
            delay.setOnFinished(event -> {
                ...
                int slotIndex = Math.max(0, Math.min(SLOT_COUNT - 1,
                        (int) ((currentX - slotStartX) / SLOT_WIDTH))); // Ensure slotIndex is within bounds
                ...
                double finalY = 780 - (slots[slotIndex]) * (BALL_RADIUS * 2 + 5); // Adjust for stacking height

                // Smooth transition to the final slot position
                double controlY = finalY - 80; // Control point for the curve
                path.getElements().add(new QuadCurveTo(
                        (slotCenterX + currentX) / 2, controlY,
                        slotCenterX, finalY
                ));

                // Update slot count to reflect ball stacking
                slots[slotIndex]++;
